...
<template>
  <div
    class="flex flex-col h-screen max-h-screen overflow-hidden bg-[#020817] p-4 sm:p-6"
  ...
  >
    ...
    <!-- CARD GAME -->
    <div v-else-if="game.verse" class="flex-1 flex flex-col min-h-0">

      <!-- FLIP CARD WRAPPER -->
      <div class="perspective-1200 flex-1 min-h-0 mb-4">
        <div
          class="relative w-full h-full transform-3d transition-transform duration-500"
          :class="{ flipped: game.isFlipped }"
        >

          <!-- FRONT -->
          <div class="backface-hidden absolute inset-0 flex flex-col bg-[#0F172A] rounded-[2rem] border border-slate-800 overflow-hidden">
            <!-- Testament badge -->
            <div class="flex justify-between items-start p-5 flex-shrink-0">
              <span class="text-slate-600 text-7xl font-serif leading-none select-none -mt-2 -ml-1">"</span>
              <span
                class="text-xs font-bold px-3 py-1 rounded-lg border uppercase tracking-wider"
                :class="game.verse.testament === 'Old'
                  ? 'bg-amber-900/30 border-amber-500/40 text-amber-400'
                  : 'bg-indigo-900/30 border-indigo-500/40 text-indigo-400'"
              >{{ game.verse.testament === 'Old' ? 'Antigo T.' : 'Novo T.' }}</span>
            </div>

            <div class="flex-1 overflow-y-auto custom-scrollbar flex items-center px-6 pb-6">
              <p class="text-xl sm:text-2xl font-semibold italic leading-relaxed text-slate-100 text-center w-full">
                {{ game.verse.text }}
              </p>
            </div>
          </div>
          ...
          <!-- BACK -->
          <div
            class="backface-hidden rotate-y-180 absolute inset-0 flex flex-col items-center justify-center bg-[#0F172A] rounded-[2rem] border-2 overflow-hidden"
            :class="game.isCorrect ? 'border-green-500/60' : 'border-rose-500/60'"
          >
            <!-- Glow -->
            <div
              class="absolute inset-0 opacity-10 rounded-[2rem]"
              :class="game.isCorrect ? 'bg-green-500' : 'bg-rose-500'"
            />

            <div class="relative z-10 text-center px-8">
              <div
                class="w-24 h-24 rounded-full flex items-center justify-center text-5xl mx-auto mb-4 border-4 animate-scale-in"
                :class="game.isCorrect ? 'bg-green-950 border-green-500' : 'bg-rose-950 border-rose-500'"
              >
                {{ game.isCorrect ? '🎯' : '💀' }}
              </div>

              <h3 class="text-4xl font-black uppercase mb-1" :class="game.isCorrect ? 'text-green-400' : 'text-rose-400'">
                {{ game.isCorrect ? 'PERFEITO!' : 'FALHOU!' }}
              </h3>

              <p v-if="game.isCorrect && game.streak > 1" class="text-orange-400 font-black text-lg mb-4">
                Combo {{ game.streak }}x 🔥
              </p>

              <div class="bg-slate-900 border border-slate-700 rounded-2xl px-8 py-4 mt-4 mb-6">
                <p class="font-black text-2xl text-white">{{ game.verse.book_name }}</p>
                <p class="text-xl font-bold text-indigo-400">{{ game.verse.chapter }}:{{ game.verse.verse_number }}</p>
              </div>

              <button
                @click="game.fetchVerse()"
                class="w-full bg-slate-700 hover:bg-slate-600 text-white font-black text-lg py-4 px-10 rounded-2xl transition-all active:scale-95"
              >
                Próximo Versículo →
              </button>
            </div>
      ...
      <div class="flex-shrink-0">
        ...
        <div v-if="game.isHintMode" class="animate-fade-in">
          ...
          <p class="text-center text-white font-black text-2xl mb-4">{{ game.hintTitle }}</p>
        ...
        <div v-else>
          ...
          <div class="flex items-center justify-between mb-3">
            ...
            <span class="text-sm font-black text-slate-400 uppercase tracking-wide text-center">
              {{ game.selectionLabel }}
            </span>
            ...
            <button
              ...
              class="flex items-center gap-1.5 px-3 py-2 rounded-xl border font-bold text-sm transition-all"
          ...
          <!-- Book selection -->
          <div v-if="game.step === 1" class="max-h-48 overflow-y-auto custom-scrollbar animate-fade-in">
            <div class="mb-3">
              <p class="text-[10px] font-black uppercase tracking-[2px] text-amber-500/70 mb-2 flex items-center gap-1">
                <span class="w-0.5 h-3 bg-amber-500 rounded inline-block" /> Antigo Testamento
              </p>
              <div class="flex flex-wrap gap-2">
                <button v-for="b in game.oldTestament" :key="b.id" @click="game.selectBook(b)"
                  class="px-3 py-2 bg-slate-800 hover:bg-amber-900/40 hover:border-amber-500/50 text-sm font-bold rounded-xl border border-slate-700 transition-all active:scale-95">
                  {{ b.name }}
                </button>
              </div>
            </div>
            <div>
              <p class="text-[10px] font-black uppercase tracking-[2px] text-indigo-500/70 mb-2 flex items-center gap-1">
                <span class="w-0.5 h-3 bg-indigo-500 rounded inline-block" /> Novo Testamento
              </p>
              <div class="flex flex-wrap gap-2">
                <button v-for="b in game.newTestament" :key="b.id" @click="game.selectBook(b)"
                  class="px-3 py-2 bg-slate-800 hover:bg-indigo-900/40 hover:border-indigo-500/50 text-sm font-bold rounded-xl border border-slate-700 transition-all active:scale-95">
                  {{ b.name }}
                </button>
              </div>
            </div>
          ...
          <!-- Chapter grid -->
          <div v-if="game.step === 2" class="max-h-48 overflow-y-auto custom-scrollbar animate-fade-in">
          ...
          <!-- Verse grid -->
          <div v-if="game.step === 3" class="max-h-48 overflow-y-auto custom-scrollbar animate-fade-in">
            <div class="grid grid-cols-5 gap-2">
          ...
          <!-- Confirm / Reveal -->
          <div v-if="game.step === 4" class="animate-fade-in">
            <div class="bg-slate-800 border border-indigo-500/30 rounded-2xl p-4 text-center mb-4">
              <p class="font-black text-xl text-white">{{ game.selection.book?.name }}</p>
              <p class="text-indigo-400 font-bold text-lg">{{ game.selection.chapter }}:{{ game.selection.verse }}</p>
            </div>
            <button
    ...
    <div v-else class="flex-1 flex flex-col items-center justify-center gap-4">
      <p class="text-4xl">😔</p>
      <p class="text-slate-400">Nenhum versículo encontrado</p>
      <button @click="game.fetchVerse()" class="px-6 py-3 rounded-xl bg-indigo-600 text-white font-bold">Tentar novamente</button>
    </div>
